...
When we delete a file in Linux, it is not immediately removed from the disk. What is removed is the entry in the directory (the pointer that references the file). The inode will continue to exist as long as some process is still using that file.
